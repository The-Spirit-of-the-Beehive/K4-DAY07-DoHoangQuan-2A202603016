...
class EmbeddingStore:
    ...
    def _search_records(self, query: str, records: list[dict[str, Any]], top_k: int) -> list[dict[str, Any]]:
        ...
        scored_records = []
        for rec in records:
            sim = _dot(query_vec, rec["embedding"])
            result_item = {k: v for k, v in rec.items() if k != "embedding"}
            result_item["score"] = sim
            result_item["similarity"] = sim
            scored_records.append((sim, result_item))
    ...
    def search_with_filter(self, query: str, top_k: int = 3, metadata_filter: dict = None) -> list[dict]:
        ...
        if not metadata_filter: candidates = self._store
        else:
            candidates = [
                rec for rec in self._store
                if all(
                    rec.get("metadata", {}).get(k) == v for k, v in metadata_filter.items()
                )
            ]
